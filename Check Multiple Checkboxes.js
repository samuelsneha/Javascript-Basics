window.onload = function () {
  ...
  formSubmit.addEventListener("submit", function (event) {
    //submit event fires on form only, not on button/input
  ...
  ulBox.addEventListener("click", function (event) {
    ...
    if (event.shiftKey) {
      ...
      let newArr = ulLi.slice(last_index, index);
      console.log(last_index, index, newArr);
      if (newArr.length > 1) deleteAllFunc(newArr);
      // console.log(newArr);
      newArr.forEach(function (element) {
        element.classList.add("toggleClass", "strikeThrough");
      });
    }
    last_index = event.target.getAttribute("data-index");
